...
1. 日付を取得（`date +%Y-%m-%d`）。
2. `./marketing/drafts/<日付>-<柱orトピック>.md` に Write で保存。各下書きは**そのままコピペできる本文**を独立ブロックで置き、メタは `<!-- -->` で添える（コピペに含まれない）。スレッドは `---` 区切り。
3. 投稿後、本文を公開リポ `niqostudio` の `marketing/history/` に記録するよう促す（別途コミット）。
